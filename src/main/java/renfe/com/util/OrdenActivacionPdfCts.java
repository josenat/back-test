package renfe.com.util;

import java.awt.Color;

import com.lowagie.text.Element;


public abstract class OrdenActivacionPdfCts {

    // IMAGES
    public static final String LOGO = "";
    
    // DEFAULT VALUES
    public static final String DEFAULT_FONT_FAMILY = "Arial";
    public static final float DEFAULT_FONT_SIZE = 10;
    public static final int DEFAULT_ALIGNMENT = Element.ALIGN_JUSTIFIED;

    public static final Color COLOR_FONDO = new Color(140,0,75);
    public static final Color COLOR_FONDO_TABLA = new Color(220, 220, 220);
    public static final Color COLOR_BLANCO = new Color(255,255,255);
    
    public static final String DATE_NOT_FILL = "__________";

    // CTS BUNDLE
    public static final String BUNDLE_NAME = "locale/OrdenActivacion";

    // CONSTANTES FIJAS
    public static final String BUNDLE_CTS_0_1 				= "ordenActivacion.text.0_1";
    public static final String BUNDLE_CTS_0_2 				= "ordenActivacion.text.0_2";
    public static final String BUNDLE_CTS_0_3 				= "ordenActivacion.text.0_3";
    public static final String BUNDLE_CTS_0_4 				= "ordenActivacion.text.0_4";
    public static final String BUNDLE_CTS_0_5 				= "ordenActivacion.text.0_5";
    public static final String BUNDLE_CTS_0_6 				= "ordenActivacion.text.0_6";
    public static final String BUNDLE_CTS_0_7 				= "ordenActivacion.text.0_7";
    
    public static final String BUNDLE_CTS_2 					= "ordenActivacion.text.2";
    
    public static final String BUNDLE_CTS_4 					= "ordenActivacion.text.4";
    
    public static final String BUNDLE_CTS_6 					= "ordenActivacion.text.6";
    
    public static final String BUNDLE_CTS_7 					= "ordenActivacion.text.7";
    
    public static final String BUNDLE_CTS_8_1				= "ordenActivacion.text.8_1";
    public static final String BUNDLE_CTS_8_2				= "ordenActivacion.text.8_2";
    public static final String BUNDLE_CTS_8_3				= "ordenActivacion.text.8_3";
    public static final String BUNDLE_CTS_8_4				= "ordenActivacion.text.8_4";
    public static final String BUNDLE_CTS_8_5				= "ordenActivacion.text.8_5";

    public static final String BUNDLE_CTS_9 					= "ordenActivacion.text.9";

    public static final String BUNDLE_CTS_10 				= "ordenActivacion.text.10";

    public static final String BUNDLE_CTS_11 				= "ordenActivacion.text.11";

    public static final String BUNDLE_CTS_12_1 				= "ordenActivacion.text.12_1";
    public static final String BUNDLE_CTS_12_2 				= "ordenActivacion.text.12_2";
    
    public static final String BUNDLE_CTS_P2_0 				= "ordenActivacion.text.2.0";
    
    public static final String BUNDLE_CTS_P2_1 				= "ordenActivacion.text.2.1";
    
    public static final String BUNDLE_CTS_P2_2 				= "ordenActivacion.text.2.2";
    
    public static final String BUNDLE_CTS_P2_3 				= "ordenActivacion.text.2.3";

    public static final String BUNDLE_CTS_P2_4 				= "ordenActivacion.text.2.4";

    public static final String BUNDLE_CTS_P2_5 				= "ordenActivacion.text.2.5";
    public static final String BUNDLE_CTS_P2_5_1 			= "ordenActivacion.text.2.5_1";
    public static final String BUNDLE_CTS_P2_5_2				= "ordenActivacion.text.2.5_2";
    public static final String BUNDLE_CTS_P2_5_3				= "ordenActivacion.text.2.5_3";
    public static final String BUNDLE_CTS_P2_5_4				= "ordenActivacion.text.2.5_4";
    
    public static final String BUNDLE_CTS_P2_6 				= "ordenActivacion.text.2.6";
    
    public static final String BUNDLE_CTS_P2_7 				= "ordenActivacion.text.2.7";
    
    public static final String BUNDLE_CTS_P2_8 				= "ordenActivacion.text.2.8";

    public static final String BUNDLE_CTS_P2_9 				= "ordenActivacion.text.2.9";
    
	public static final int ID_DOCUMENTO						= 2;
	
    // CONSTANTES DEFINIDAS
	public static final String C_001_NOMBRE_CABECERA			= "001_NOMBRE_CABECERA";
	public static final String C_002_CARGO_CABECERA				= "002_CARGO_CABECERA";
	public static final String C_003_DESTINATARIO_NOMBRE		= "003_DESTINATARIO_NOMBRE";
	public static final String C_004_DESTINATARIO_CARGO			= "004_DESTINATARIO_CARGO";
    public static final String C_005_DESTINATARIO_AREA			= "005_DESTINATARIO_AREA";
    public static final String C_006_CC_NOMBRE					= "006_CC_NOMBRE";
    public static final String C_007_CC_CARGO					= "007_CC_CARGO";
    public static final String C_008_CC_AREA					= "008_CC_AREA";
    public static final String C_009_ESTIMADO_NOMBRE			= "009_ESTIMADO_NOMBRE";
    public static final String C_010_ORDENANTE					= "010_ORDENANTE";
    public static final String C_011_FIRMADO_AREA				= "011_FIRMADO_AREA";
    
    // VARIABLES USUARIO
    public static final Integer V_NUMERO_DOCUMENTO				= new Integer(1);
    public static final Integer V_ANYO_REFERENCIA				= new Integer(2);
    public static final Integer V_FECHA_DOCUMENTO				= new Integer(3);
    public static final Integer V_ASUNTO						= new Integer(4);
    public static final Integer V_FECHA_EFECTIVA				= new Integer(5);
    public static final Integer V_MOTIVO_ACTIVACION				= new Integer(6);

    // VARIABLES ORDEN SUSPENSION
    public static final Integer V_FECHA_ENTRADA_SUSPENSION		= new Integer(5);
    public static final Integer V_FECHA_EFECTIVA_SUSPENSION		= new Integer(6);
    public static final Integer V_NOMBRE_ORDENANTE				= new Integer(7);
    public static final Integer V_CARGO_ORDENANTE				= new Integer(8);
    public static final Integer V_DEPARTAMENTO_ORDENANTE		= new Integer(9);
    public static final Integer V_NOMBRE_DESTINATARIO				= new Integer(10);
    public static final Integer V_CARGO_DESTINATARIO				= new Integer(11);
    public static final Integer V_DIRECCION_GENERAL_DESTINATARIO	= new Integer(12);
    public static final Integer V_NOMBRE_PILA_DESTINATARIO		= new Integer(13);
    public static final Integer V_RESPONSABLE_GERENCIA_TERRITORIAL	= new Integer(14);
    public static final Integer V_NOMBRE_GERENCIA_TERRITORIAL		= new Integer(15);
    public static final Integer V_DIRECCION_GERENCIA_TERRITORIAL	= new Integer(16);
    public static final Integer V_ARTICULO_ACTIVACION				= new Integer(17);
    public static final Integer V_HABILITACION_SUSPENSION			= new Integer(-1);
    
    
}
